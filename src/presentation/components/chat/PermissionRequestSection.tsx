...
export const PermissionRequestSection = React.memo(function PermissionRequestSection({
	permissionRequest,
	toolCallId,
	acpClient,
		...
		>
			...
			{isSelected && selectedOption && (
				...
				>
					...
					<span className="sr-only">Approved:</span> Selected: {selectedOption.name}
				</div>
			)}
			{isCancelled && (
				<div
				...
				>
					<span aria-hidden="true">&#9888;</span>
					<span className="sr-only">Warning:</span> Cancelled: Permission request was cancelled
				</div>
			)}
		</div>
	);
