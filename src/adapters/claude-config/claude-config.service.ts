...
export class ClaudeConfigService {
	...
	async getSettings(): Promise<ClaudeSettings> {
		...
		// Read MCP servers
		const mcpServers = await this.readJsonFile<{
	...
	async addMcpServer(config: McpServerConfig): Promise<void> {
		...
		const existing = settings.mcpServers.findIndex(
	...
	private async readJsonFile<T>(filename: string): Promise<T | null> {
		...
		try {
			...
			if ((error as NodeJS.ErrnoException).code !== "ENOENT") {
				this.logger.error(
					`[ClaudeConfigService] Failed to read ${filename}:`,
					error,
				);
			}
			return null;
	...
	private startWatching(): void {
		...
		try {
			this.watcher = fs.watch(this.configDir, (eventType, filename) => {
				if (
					filename &&
					Object.values(CONFIG_FILES).includes(
						filename as (typeof CONFIG_FILES)[keyof typeof CONFIG_FILES],
					)
				) {
			...
			this.logger.error(
				"[ClaudeConfigService] Failed to start watching:",
				error,
			);
		}
	}
	...
	private notifyListeners(settings: ClaudeSettings): void {
		for (const listener of this.listeners) {
			try {
				...
				this.logger.error(
					"[ClaudeConfigService] Listener error:",
					error,
				);
			}
		}
